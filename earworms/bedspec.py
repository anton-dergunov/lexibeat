...
@dataclass
class Lead:
    instrument: str = "synth"  # "synth" or "piano"
    level: float = 1.0
...
class BedSpec:
    ...
    def chord(self, degree: int) -> list[int]:
        """A spread five-note voicing: root, fifth, octave, third, twelfth."""
        steps = self.scale_steps()
        n = len(steps)
...
def _yoga(rng: random.Random) -> BedSpec:
    ...
    return BedSpec(
        bpm=rng.choice([76, 78, 80, 82, 84]),
        root=rng.choice([43, 45, 47, 48]),
        scale=rng.choice(["natural_minor", "dorian"]),
        progression=rng.choice([[0, 5, 2, 6], [0, 3, 5, 4], [0, 5, 3, 6]]),
        pad=Pad(cutoff_base=rng.uniform(800, 1100), cutoff_motion=rng.uniform(300, 500)),
        drums=Drums(kick=rng.choice(["x.......x.......", "x.......x...x..."]),
                    rim=rng.choice(["....x.......x...", "....x..........."])),
        lead=Lead(bar_probability=rng.uniform(0.35, 0.55), max_notes=2),
...
def _nocturne(rng: random.Random) -> BedSpec:
    ...
    return BedSpec(
        bpm=rng.choice([60, 64, 66, 68]),
        root=rng.choice([40, 41, 43, 45]),
        scale=rng.choice(["natural_minor", "harmonic_minor"]),
        progression=rng.choice([[0, 4, 5, 0], [0, 6, 5, 4], [0, 2, 5, 4]]),
        pad=Pad(level=0.6, cutoff_base=rng.uniform(600, 850), overlap=rng.uniform(1.6, 2.1)),
        bass=Bass(level=0.42, attack=0.25),
        drums=Drums(kick="x.......x.......", rim="........x.......",
                    shaker_level=0.03, shaker_density=0.5),
        lead=Lead(instrument="piano", level=1.1,
...
def _lofi(rng: random.Random) -> BedSpec:
    ...
    return BedSpec(
        ...
        pad=Pad(level=0.45, cutoff_base=rng.uniform(500, 750), cutoff_motion=250),
        bass=Bass(level=0.58, attack=0.08, decay_bars=0.45),
        drums=Drums(kick="x.....x...x.....", rim="....x.......x...",
                    kick_level=0.6, rim_level=0.2,
                    shaker_level=rng.uniform(0.05, 0.09)),
        lead=Lead(instrument="piano", bar_probability=rng.uniform(0.4, 0.65),
...
def _warm(rng: random.Random) -> BedSpec:
    ...
    return BedSpec(
        bpm=rng.choice([84, 88, 90, 92]),
        root=rng.choice([48, 50, 53, 55]),
        scale=rng.choice(["major", "lydian"]),
        progression=rng.choice([[0, 4, 5, 3], [0, 3, 4, 0], [0, 5, 3, 4]]),
        pad=Pad(cutoff_base=rng.uniform(1000, 1400), cutoff_motion=rng.uniform(350, 550)),
        bass=Bass(level=0.45),
        drums=Drums(kick="x...x...x...x...", rim="....x.......x...",
                    shaker_level=rng.uniform(0.05, 0.08)),
        lead=Lead(instrument="piano", bar_probability=rng.uniform(0.45, 0.7),
                  max_notes=2, register=(24, 38)),
